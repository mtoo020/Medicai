...
with tf.Graph().as_default():
	...
	filter_ = tf.placeholder(tf.float32)
	...
	correct_prediction = tf.equal(tf.cast(vgg.prob[:,1]<filter_,tf.int64), tf.argmax(true_out,1))
	...
	sum_ = 0
	for i in xrange(9000,10000,100):
		fil = float(i)/10000
		#fil = 0.9995
		sum_ = 0
		for i in xrange(0,1000,100):
			sum_+=accuracy.eval(session=sess, feed_dict={
	    images: imgs_val[range(i,i+100), :, :, :], true_out: lbls_val[range(i,i+100)], train_mode: False, filter_:fil})
